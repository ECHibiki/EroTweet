...
function generateMediaSingatureAPPEND($request, $message, $params, $secrets){	
  // BUILD SIGNATURE
	...
	$paramter_string = urlEncodeCustom("command=" . $message["command"] . "&media=" . urlEncodeCustom($message["media"]) . "&media_id="  .  $message["media_id"] . "&oauth_consumer_key=" . $params["oauth_consumer_key"]
	 . "&oauth_nonce=" . $params["oauth_nonce"] . "&oauth_signature_method=" . $params["oauth_signature_method"] . "&oauth_timestamp=" . $params["oauth_timestamp"]."&oauth_token=" . $params["oauth_token"] .
	...
	$secret_string = $secrets["consumer_secret"] . "&" . $secrets["oauth_secret"];
	echo $secret_string. "<br/><br/>";;
	$signature =  urlencode(base64_encode(hash_hmac("SHA1",$base_string, $secret_string, true)));	
...
function generateMediaSingatureFINAL($request, $message, $params, $secrets){	
  // BUILD SIGNATURE
	$request_method = strtoupper($request["request_method"]);
	$base_url = urlEncodeCustom($request["base_url"]);
	//$message["media_data"] = 123;
	$paramter_string = urlEncodeCustom("command=" . $message["command"] . "&media_id="  .  $message["media_id"] . "&oauth_consumer_key=" . $params["oauth_consumer_key"]
	 . "&oauth_nonce=" . $params["oauth_nonce"] . "&oauth_signature_method=" . $params["oauth_signature_method"] . "&oauth_timestamp=" . $params["oauth_timestamp"]."&oauth_token=" . $params["oauth_token"] .
	 "&oauth_version=" .$params["oauth_version"]); 
...
"Authorization: OAuth oauth_consumer_key=\"$consumer_key\",oauth_nonce=\"$random_value\",oauth_signature=\"$signature\",oauth_signature_method=\"$method\",oauth_timestamp=\"$timestamp\",oauth_token=\"$access_token\",oauth_version=\"$oauth_version\""																				
	...
	$postfield_string = "media=" . urlencode($binary_file) . "&oauth_consumer_key=$consumer_key&oauth_nonce=$random_value&oauth_signature=$signature&oauth_signature_method=$method&oauth_timestamp=$timestamp&oauth_token=$access_token&oauth_version=$oauth_version";
						//&command=$command&media_id=$media_id&segment_index=$segment_index";
...
"Authorization: OAuth oauth_consumer_key=\"$consumer_key\",oauth_nonce=\"$random_value\",oauth_signature=\"$signature\",oauth_signature_method=\"$method\",oauth_timestamp=\"$timestamp\",oauth_token=\"$access_token\",oauth_version=\"$oauth_version\""																				
	...
	echo $http_response;
		...
		$signature =  generateMediaSingatureFINAL(array(
								"base_url" => $media_api,
								"request_method" => "POST"
								),
							array(
